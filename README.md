...
프로시저럴 생성 지형 위에서 끊임없이 밀려오는 적을 처치하고, 업그레이드를 쌓아 25분간 생존하는 3인칭 슈터 로그라이크 게임입니다. 시간 정지와 타임 스킵 스킬을 전략적으로 활용해 위기를 극복합니다.
...
| 언어 | C# |
| 개발 인원 | 1인 개발 |
| 개발 기간 | 2025.03 ~ 2025.03 |
| 렌더링 | Universal Render Pipeline |
| 해상도 | 1920x1080, 적응형 스케일링 |

## 게임 플레이

- 프로시저럴 생성된 지형에서 시작 → 적이 웨이브로 스폰 → 처치하여 에너지 수집 → 업그레이드 카드 선택 → 난이도 상승 반복
- 60초마다 난이도가 상승하며, 25단계(약 25분)를 생존하면 클리어
...
- **난이도 스케일링**: 60초마다 `scalingFactor = 1 + (level * 0.1)`로 가속적 난이도 증가 — 스폰 간격 감소, 최대 적 수/체력/데미지 배율 증가
...
- **Observer** — EnemyDeathNotifier의 `System.Action OnDeath` 콜백으로 스포너에 사망 통지

### 렌더링

- Universal Render Pipeline (URP)
- Post Processing Volume (ColorAdjustments, ChromaticAberration, Vignette, LensDistortion)
- Custom Shader Graph (지형 그라디언트, Volumetric Fog, 적 투명도 페이드)
...
│   ├── Terrain/
│   │   ├── OptimizedTerrainGenerator.cs     # 프로시저럴 지형 생성 (멀티스레드)
