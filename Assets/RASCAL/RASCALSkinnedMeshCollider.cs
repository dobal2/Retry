    ...
    void ProcessInternal(bool cleanOld, bool async) {
        ...
        IEnumerable<bool> ApplyProcessingRoutine(bool async) {
            foreach (Skinfo skin in skinfos) {
                ...
                int vc = skin.skinnedMesh.sharedMesh.vertexCount;
                Mesh sMesh = skin.skinnedMesh.sharedMesh;

                List<Vector3> sNorm = new List<Vector3>(vc); sMesh.GetNormals(sNorm);

                List<Color32> sClrs = new List<Color32>(vc); sMesh.GetColors(sClrs);

                List<Vector2> sUVs0 = new List<Vector2>(vc); sMesh.GetUVs(0, sUVs0);
                List<Vector2> sUVs1 = new List<Vector2>(vc); sMesh.GetUVs(1, sUVs1);
                List<Vector2> sUVs2 = new List<Vector2>(vc); sMesh.GetUVs(2, sUVs2);
                List<Vector2> sUVs3 = new List<Vector2>(vc); sMesh.GetUVs(3, sUVs3);
                ...
                foreach (BoneInfo boneInfo in skin.bones) {
                    foreach (BoneMeshColl bone in boneInfo.boneMeshes) {
                        ...
                        var cMesh = bone.collMesh;
                        cMesh.SetNormals(bone.CopyMeshData(sNorm));

                        cMesh.SetColors(bone.CopyMeshData(sClrs));

                        cMesh.SetUVs(0, bone.CopyMeshData(sUVs0));
                        cMesh.SetUVs(1, bone.CopyMeshData(sUVs1));
                        cMesh.SetUVs(2, bone.CopyMeshData(sUVs2));
                        cMesh.SetUVs(3, bone.CopyMeshData(sUVs3));
                    }
        ...
        public class BoneMeshColl {
            ...
            internal List<T> CopyMeshData<T>(List<T> source) {
                if (source.Count == 0) {
                    return null;
                }
                List<T> copy = new List<T>(distinctVerts.Length);
                for (int i = 0; i < distinctVerts.Length; i++) copy.Add(source[distinctVerts[i]]);
                return copy;
            }
            ...
            public void Update(List<Vector3> bakedVerts, bool force = false) {
                ...
                if (thresh) {
                    ...
                    if (saveColMeshToFile) {
                        ...
                        char[] illegalChars = { '/', '\\', '*', '?', '"', ':', '<', '>', '|' };
                        fileName = string.Join("-", fileName.Split(illegalChars));
                        ...
                        try {
                            ...
                            Debug.LogError($"RASCAL: Error in creation of mesh file: [{dir}]  [{e.Message}]");
                        }
                    }
                    else
#endif
