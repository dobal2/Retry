...
public class GrassPainterWindow : EditorWindow
{
    ...
    private static bool pendingAutoReset = false;
    ...
    private static void OnEditorReload()
    {
        ...
        EditorApplication.delayCall += TryOpenWindowForReset;
    }
    
    private static void TryOpenWindowForReset()
    {
        if (pendingAutoReset)
        {
            var window = GetWindow<GrassPainterWindow>();
            window.Show(); // 창이 안 떠 있으면 강제로 띄움
            window.TryAutoReset();
    ...
    public void TryAutoReset()
    {
        ...
        {
            grassCompute.Reset();
            Debug.Log("[GrassPainter] Auto Reset 실행됨.");
        }
    }
    ...
    // public void UpdateMaterialOnly(Material newMat)
    // {
    //     if (renderers == null) return; // grass 인스턴스 렌더러 리스트
    //
    //     foreach (var r in renderers)
    //     {
    //         r.sharedMaterial = newMat; // 또는 .material = newMat; (인스턴스화됨)
    //     }
    //
    //     // 컴퓨트 셰이더 사용하는 경우:
    //     // computeShader.SetTexture(kernelID, "_YourTextureName", newMat.GetTexture("_MainTex"));
    // }
    ...
    private void OnGUI()
    {
        ...
        {
            EditorGUILayout.HelpBox("Tool Settings가 비어있습니다. Grass Tool Settings Asset을 생성하거나 연결하세요.", MessageType.Warning);
            if (GUILayout.Button("Create Default Tool Settings"))
            {
        ...
        // Material newMat = (Material)EditorGUILayout.ObjectField("Grass Material", grassCompute.currentPresets.materialToUse, typeof(Material), false);
        // if (newMat != grassCompute.currentPresets.materialToUse)
        // {
        //     grassCompute.currentPresets.materialToUse = newMat;
        //     grassCompute.UpdateMaterialOnly(newMat);  // 전체 리셋 없이 머티리얼만 교체
        //     Debug.Log("[GrassPainter] Material updated without full reset.");
        // }
    ...
    private static void CheckAutoReset()
    {
        ...
        var window = GetWindow<GrassPainterWindow>();
        if (window != null && window.grassCompute != null)
        {
            window.grassCompute.Reset();
            pendingAutoReset = false;
            EditorApplication.update -= CheckAutoReset;
        }
    }
